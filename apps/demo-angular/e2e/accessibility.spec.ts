...
async function selectAll(page: Page) {
  await page.evaluate(() => {
    ...
    const pm = document.querySelector('domternal-editor .ProseMirror');
...
test.describe('Bubble menu — ARIA attributes', () => {
  ...
  test('bubble menu buttons have aria-label', async ({ page }) => {
  ...
  test('bubble menu buttons have aria-pressed', async ({ page }) => {
    const buttons = page.locator(`${bubbleMenu} button`);
    const count = await buttons.count();
    for (let i = 0; i < count; i++) {
  ...
  test('bubble menu separators have role="separator"', async ({ page }) => {
    const seps = page.locator(`${bubbleMenu} [role="separator"]`);
    const count = await seps.count();
    expect(count).toBeGreaterThanOrEqual(1);
  });

  test('aria-pressed="true" on bold button when bold text selected', async ({ page }) => {
    await setContent(page, '<p><strong>Bold text</strong></p>');
    await selectAll(page);
    await page.waitForTimeout(200);

    await expect(page.locator(`${bubbleMenu} button[title="Bold"]`)).toHaveAttribute('aria-pressed', 'true');
  });

  test('aria-pressed="false" on bold button when plain text selected', async ({ page }) => {
    await expect(page.locator(`${bubbleMenu} button[title="Bold"]`)).toHaveAttribute('aria-pressed', 'false');
  });
});
...
test.describe('Toolbar — dropdown ArrowDown/ArrowUp', () => {
  ...
  test('Escape closes dropdown and returns focus to trigger', async ({ page }) => {
    ...
    // Focus should be back on a toolbar button
  ...
  test('ArrowDown on non-dropdown button does nothing', async ({ page }) => {
    ...
    // Bold is not a dropdown trigger, so focus should stay
...
test.describe('Emoji picker — grid keyboard navigation', () => {
  ...
  test('ArrowDown moves focus down one row (8 columns)', async ({ page }) => {
    ...
    const focusedLabel = await page.evaluate(() => document.activeElement?.getAttribute('aria-label'));
    // Should be 8 positions ahead (next row, same column)
...
test.describe('Table cell toolbar — ARIA', () => {
  ...
  test('cell toolbar has role="toolbar" when visible', async ({ page }) => {
    // Select multiple cells to trigger cell toolbar
    await page.evaluate((sel) => {
      ...
      // Select all cells via selectAll
      comp.editor.commands.selectAll();
    }, editorSelector);
    await page.waitForTimeout(300);
...
test.describe('Focus-visible — keyboard focus indicators', () => {
  ...
  test('toolbar button does not show outline on mouse click', async ({ page }) => {
    ...
    const outline = await btn.evaluate((el) => getComputedStyle(el).outlineStyle);
    // After mouse click, focus-visible should not apply, so outline should be none
    // (Chromium treats mouse click as non-keyboard focus)
...
test.describe('prefers-reduced-motion', () => {
  test('animations are disabled when reduced motion is preferred', async ({ page }) => {
    ...
    // Open a dropdown to check if fade-in animation is disabled
    const trigger = page.locator(`${toolbar} button[aria-label="Highlight"]`);
    await trigger.click();
    await page.waitForTimeout(100);
